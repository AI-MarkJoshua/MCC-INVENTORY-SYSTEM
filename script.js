...
async function updateCategoryDropdowns() {
    ...
    ['item-category', 'edit-category'].forEach(selId => {
        ...
        const defaultOpts = ['Chain','Seat / Saddle','Wheels','Brakes','Pedals','Handlebars','Frame','Tires','Gears / Derailleur','Lights','Other'];
        Array.from(sel.options).forEach(opt => {
            if (opt.value && opt.value !== '__custom__' && !defaultOpts.includes(opt.text))
...
async function createAccount() {
    ...
    const name = $('new-name').value.trim(), email = $('new-email').value.trim();
    const role = $('new-role').value, pass = $('new-password').value;
    ...
    if (data.user) {
        const { error: pe } = await supabaseClient.from('profiles').upsert({ id: data.user.id, full_name: name, email, role });
...
async function loadAccounts() {
    ...
    if (!data.length) { tbody.innerHTML = `<tr><td colspan="4" class="empty-row">No accounts found.</td></tr>`; return; }
...
function openEditModal(id) {
    ...
    const sel  = $('edit-category');
    const opts = Array.from(sel.options).map(o => o.value);
    if (opts.includes(item.category)) {
...
function setupItemNameAutoComplete() {
    ...
    input.addEventListener('input', function() {
        ...
        const filtered=v?allItems.filter(i=>i.name.toLowerCase().includes(v.toLowerCase())):allItems;
        updateDatalist(filtered);
        dd.classList.toggle('active', !!v||false);
        wrap.classList.toggle('active', !!v||false);
            ...
            plugins:{ legend:{display:false},
                tooltip:{ callbacks:{
                    label: ctx=>{
                        ...
                        return [`Quantity: ${q}`, `Status: ${q===0?'Out of Stock':q<=5?'Low Stock':'In Stock'}`];
                    }
                }}
            },
            scales:{
...
function initReportDefaults() {
    ...
    if ($('report-start') && !$('report-start').value) $('report-start').value = firstOfMonth;
    if ($('report-end')   && !$('report-end').value)   $('report-end').value   = today;
...
async function generateReport() {
    ...
    if (!startDate || !endDate) return setMsg('report-error', 'Please select both start and end dates.');
    if (startDate > endDate)    return setMsg('report-error', 'Start date cannot be after end date.');

    const btn = document.querySelector('#view-reports .btn-primary');
    const t=btn.querySelector('.btn-text'), l=btn.querySelector('.btn-loader');
    t.style.display='none'; l.style.display=''; btn.disabled=true;

    // Build query — end date needs +1 day to be inclusive
...
function renderReport(transactions, type, startDate, endDate) {
    ...
    tbody.innerHTML = transactions.map(tx => {
        const remarksBadge = tx.type==='stock_in'
        ...
        return `<tr>
            ...
            <td>
                <strong>${escHtml(tx.user_name||'—')}</strong>
                <span class="role-chip">${escHtml(tx.user_role||'—')}</span>
            </td>
...
function printReport() {
    ...
    <head>
        <meta charset="UTF-8">
        <title>MCC Inventory Report</title>
        <style>
            * { box-sizing:border-box; margin:0; padding:0; }
            ...
            .print-title { margin: 16px 0 4px; }
            .print-title h2 { font-size: 16px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; }
            ...
            thead th { background: #f8f9fb; text-align: left; padding: 8px 10px; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; color: #6b7280; border-bottom: 2px solid #e2e6ed; border-top: 1px solid #e2e6ed; }
    ...
    <body>
        <div class="print-header">
            ...
            <div class="print-org">
                <h1>MCC<span>Inventory</span></h1>
                <p>Bike Parts &amp; Supplies System</p>
            </div>
        </div>
        ...
        <div class="print-footer">
            <span>MCC Bike Inventory System</span>
