...
async function ensureCustomCategoriesTable() {
    try {
        ...
        if (error && error.code === 'PGRST116') {
            ...
            console.log('Creating custom_categories table...');
            
            // Since we can't create tables directly via client, we'll use a workaround
        ...
        console.error('Error checking custom_categories table:', error);
        return false;
    }
}
...
async function saveCustomCategory(categoryName) {
    ...
    console.log('Saving custom category to database:', trimmedName);
    ...
    try {
        ...
        if (checkError && checkError.code === 'PGRST116') {
            // Table doesn't exist
            console.error('ERROR: custom_categories table does not exist in database!');
            console.log('Please create the table using this SQL in your Supabase dashboard:');
            console.log(`
CREATE TABLE IF NOT EXISTS custom_categories (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
        ...
        if (existing) {
            // Category already exists, no need to save again
            console.log('Category already exists in database:', existing);
            return existing;
        }
        ...
        if (error) {
            console.error('Error saving custom category to database:', error);
            return null;
        }
        
        console.log('Successfully saved custom category to database:', data);
        return data;
    } catch (error) {
        console.error('Error in saveCustomCategory:', error);
        return null;
    }
}
...
async function loadCustomCategories() {
    try {
        console.log('Loading custom categories from database...');
        ...
        if (error) {
            console.error('Error loading custom categories from database:', error);
...
CREATE TABLE IF NOT EXISTS custom_categories (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
        ...
        console.log('Loaded custom categories from database:', data);
        return data || [];
    } catch (error) {
        console.error('Error in loadCustomCategories:', error);
        return [];
    }
}
...
async function updateCategoryDropdowns() {
    ...
    console.log('Found custom categories:', customCategories);
    
    // Update stock entry dropdown
    const stockSelect = $('item-category');
    const editSelect = $('edit-category');
    
    if (stockSelect) {
        console.log('Updating stock dropdown...');
        updateDropdownWithCustomCategories(stockSelect, customCategories);
    }
    
    if (editSelect) {
        console.log('Updating edit dropdown...');
        updateDropdownWithCustomCategories(editSelect, customCategories);
    }
}

function updateDropdownWithCustomCategories(selectElement, customCategories) {
    if (!selectElement) {
        console.log('Select element not found');
        return;
    }
    
    console.log('Updating dropdown with custom categories:', customCategories);
    
    // Get current selected value
    const currentValue = selectElement.value;
    
    // Remove existing custom category options (keep the "+ Add Custom Category" option)
    const options = Array.from(selectElement.options);
    options.forEach(option => {
        if (option.value && option.value !== '__custom__' && !['Chain', 'Seat / Saddle', 'Wheels', 'Brakes', 'Pedals', 'Handlebars', 'Frame', 'Tires', 'Gears / Derailleur', 'Lights', 'Other'].includes(option.text)) {
            console.log('Removing existing custom option:', option.text);
            selectElement.remove(option);
        }
    });
    
    // Add custom categories before the "+ Add Custom Category" option
    const addCustomOption = Array.from(selectElement.options).find(opt => opt.value === '__custom__');
    
    customCategories.forEach(category => {
        console.log('Adding custom category option:', category.name);
        const option = document.createElement('option');
        option.value = category.name;
        option.textContent = category.name;
        selectElement.insertBefore(option, addCustomOption);
    });
    ...
    console.log('Dropdown update complete. Total options:', selectElement.options.length);
}
...
async function initApp(user) {
    ...
    // Load custom categories first, then load items
...
async function addStock() {
    ...
    if (categorySelect.value === '__custom__' && category) {
        ...
        // Update dropdowns to include the new custom category
        await updateCategoryDropdowns();
    }
